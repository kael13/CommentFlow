import { generateReply as llmGenerateReply, isLLMAvailable } from '@commentflow/shared';
...
const replyQueue = createQueue('reply-generation');

createWorker(replyQueue, async (job) => {
  const { commentText, tone, intent, sentiment, authorName } = job.data;
  const reply = await llmGenerateReply(commentText, tone, intent, sentiment, authorName);
  if (reply) return { reply, source: 'llm' };
  const fallback = await generateWithTemplates(commentText, intent, tone, sentiment, authorName);
...
export async function generateReply(commentText, intent, tone, sentiment, authorName) {
  ...
  if (!isLLMAvailable()) {
    ...
    return generateWithTemplates(commentText, intent, tone, sentiment, authorName);
  }

  try {
    const result = await enqueueAndWait(
      replyQueue,
      { commentText, tone, intent, sentiment, authorName },
      30000
    );
    logger.info(`Reply generated via ${result.source}`);
    return result.reply;
  } catch (error) {
    logger.warn(`LLM generation failed, falling back to templates: ${error.message}`);
    return generateWithTemplates(commentText, intent, tone, sentiment, authorName);
  }
}

export async function generateWithTemplates(commentText, intent, tone, sentiment, authorName) {
  ...
  if (template && template.template_text) {
    return replaceName(template.template_text, authorName);
  }
